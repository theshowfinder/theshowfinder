...
export async function createArtistAction(formData: FormData) {
  ...
  const { data: artist, error } = await db
    .from('artists')
    .insert({ name, slug, image_url, description, tour_name, onsale_date, tickets_url, is_featured })
    .select('id, slug')
    .single()
  ...
  if (tour_name) {
    await db.from('tours').insert({ artist_id: artist.id, tour_name, onsale_date })
  }
